...
class Module(tf.Module):
  ...
  @property
  def variables(self):
    """Collect TensorFlow and Keras 3 variables in deterministic order."""
    variables = []
    seen = set()
    ...
    def visit(value):
      ...
      elif all(hasattr(value, name) for name in ('assign', 'numpy', 'shape')):
        # Keras 3 backend Variable is no longer a tf.Variable subclass.
        variables.append(value)
  ...
  def trainable_variables(self):
    return [
        ...
        if getattr(variable, 'trainable', True)]
  ...
  def get(self, name, ctor, *args, **kwargs):
    ...
    if name not in self._modules:
      ...
      # Keras 3 layers inside a plain dictionary are not discovered through
      # tf.Module.variables. Register each layer as a tracked attribute too.
      setattr(self, f'_layer_{name}', module)
  ...
  while True:
    ...
    if capacity:
      ...
      for filename in filenames:
        episode_length = int(filename.stem.rsplit('-', 1)[-1]) - 1
...
class Adam(tf.Module):
  ...
  def __init__(self, name, modules, lr, clip=None, wd=None, wdpattern=r'.*'):
    ...
    self._loss_scale = (
        1024.0 if prec.global_policy().compute_dtype == 'float16' else None)
    self._variables = None

  @property
  def variables(self):
    variables = self._opt.variables
    return variables() if callable(variables) else variables

  def __call__(self, tape, loss):
    if self._variables is None:
      variables = [module.trainable_variables for module in self._modules]
