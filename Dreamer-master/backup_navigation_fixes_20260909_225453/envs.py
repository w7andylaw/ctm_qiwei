...
# ==============================================================================
# DreamerV2-native UAV interface (replaces uav_adapter.py)
# ==============================================================================
...
class DreamerV2UAVEnv:
    ...
    def __init__(self, task="relay", size=(64, 64), max_episode_steps=100, seed=None):
        if task not in self.TASKS:
            raise ValueError(f"Unknown UAV task {task!r}")
        self.task = task
        self.size = tuple(size)
        self.seed = seed
        self.seed_on_next_reset = True
        self.env = (DirectNavigationEnv if task == "direct" else RelayNavigationEnv)(
    ...
    def action_space(self):
        # First K coordinates encode a categorical selection; second K are
        # per-action continuous parameters. Actor outputs are bounded [-1,1].
        return spaces.Box(-1.0, 1.0, shape=(2 * self.num_actions,), dtype=np.float32)
    ...
    def observation_space(self):
        ...
        for key in ("is_success", "out_of_bounds", "terminated", "truncated",
            ...
            result[key] = spaces.Box(0.0, 1.0, shape=(), dtype=np.float32)
        return spaces.Dict(result)

    @property
    def current_phase(self): return self.env.current_phase
    @property
    def num_phases(self): return self.env.num_phases
    @property
    def current_goal(self): return self.env.current_goal
    @property
    ...
    def decode_action(self, action):
        ...
        if action.shape != self.action_space.shape:
            raise ValueError(f"Expected {self.action_space.shape}, got {action.shape}")
        # HybridActionDecoder emits one-hot/ST selection. Argmax is robust for
        # random prefill actions as well.
        discrete = int(np.argmax(action[:self.num_actions]))
        parameter = np.asarray([action[self.num_actions + discrete]], np.float32)
        return discrete, parameter

    def reset(self):
        seed = self.seed if self.seed_on_next_reset else None
        self.seed_on_next_reset = False
        obs, info = self.env.reset(seed=seed)
        self.last_obs = obs
        info = dict(info, terminated=False, truncated=False)
        return self._convert_obs(obs, info)

    def step(self, action):
        hybrid = self.decode_action(action)
        obs, reward, terminated, truncated, info = self.env.step(hybrid)
        self.last_obs = obs
        done = bool(terminated or truncated)
        info = dict(info)
        info.update(discount=np.asarray(0.0 if terminated else 1.0, np.float32),
    ...
    def _convert_obs(self, obs, info=None):
        ...
        out = {
            ...
            "phase": np.asarray(self.env.current_phase, np.float32),
        }
        for key in ("is_success", "out_of_bounds", "terminated", "truncated",
                    "relay_reached", "carrying_supply"):
    ...
    def _render(self, obs):
        ...
        def pixel(point):
            point = np.asarray(point, np.float32) / self.env.map_size
            return (int(np.clip(point[0], 0, 1) * (width - 1)),
                    int((1 - np.clip(point[1], 0, 1)) * (height - 1)))
        def marker(point, color, radius=3, outline=None):
            x, y = pixel(point)
            draw.ellipse((x-radius, y-radius, x+radius, y+radius), fill=color, outline=outline)
        if self.task == "relay":
            relay_active = self.env.current_phase == 0
            marker(self.env.relay_goal, (225,160,45), 3, (255,255,255) if relay_active else None)
            marker(self.env.final_goal, (50,190,90), 4, (255,255,255) if not relay_active else None)
        else:
            marker(self.env.current_goal, (50,190,90), 4, (255,255,255))
        x, y = pixel(self.env.state.position)
        heading = float(self.env.state.heading)
        tip = (x + int(6*np.cos(heading)), y - int(6*np.sin(heading)))
        draw.line((x, y, *tip), fill=(80,180,255), width=2)
